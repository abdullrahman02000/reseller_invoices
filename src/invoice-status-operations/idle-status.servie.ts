...
@Injectable()
export class IdleInvoiceStatus implements IInvoiceStatus {
  constructor(
  ...
  async getReport(
    ...
    if (startDate == undefined || endDate == undefined) {
      ...
      if (startDate == undefined) {
        startDate = summary.minCreatedAt;
      }
      if (endDate == undefined) {
        endDate = summary.maxCreatedAt;
      }
    }
    return await this.invoiceReportRepository.find({
      where: { ...where, createdAt: Between(startDate, endDate) },
    });
  }
}
